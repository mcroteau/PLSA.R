...
public class ItemRepo {
    ...
    public List<ItemCategory> getListItems(long id, long businessId){
        String sql = "select * from item_categories where category_id = [+] and business_id = [+] order by item_id desc";
        List<ItemCategory> items = (ArrayList) dao.getList(sql, new Object[]{ id, businessId }, ItemCategory.class);
        return items;
    }
    ...
    public Boolean saveValue(OptionValue optionValue){
        String sql = "insert into option_values (item_option_id, option_value, price, quantity) values ([+],'[+]',[+],[+])";
        dao.save(sql, new Object[] {
    ...
    public List<Item> query(String query, Long id) {
