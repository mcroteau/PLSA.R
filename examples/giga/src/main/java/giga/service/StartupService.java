...
public class StartupService {
    ...
    public void run() throws Exception {
        ...
        this.cartRepo = new CartRepo(dao);
        this.saleRepo = new SaleRepo(dao);
        this.itemRepo = new ItemRepo(dao);
        this.userRepo = new UserRepo(dao);
        this.roleRepo = new RoleRepo(dao);
        this.businessRepo = new BusinessRepo(dao);
        this.categoryRepo = new CategoryRepo(dao);
        this.designRepo = new DesignRepo(dao);
        this.pageRepo = new PageRepo(dao);

        String password = SecurityManager.dirty(Giga.SUPER_PASSWORD);
    ...
    protected void mock(String password, Role savedCustomerRole, Role savedBusinessRole){
        ...
        patron.setEmail("croteau.mike+patron@gmail.com");
        ...
        ibmUser.setEmail("croteau.mike+ibm@gmail.com");
        ...
        try {
            businessService.configure(savedKopi, true, userRepo, pageRepo, designRepo);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for(int k = 0; k < 91; k++) {
            ...
            item.setName("Kilo Item " + Giga.getString(4));
            ...
            optionValue.setOptionValue("Size 10");
            ...
            optionValueDos.setOptionValue("Blue + White");
        ...
        User user = new User();
        user.setEmail("croteau.mike+graphite@gmail.com");
        user.setPassword(SecurityManager.dirty("gigabeat"));
        userRepo.save(user);
        ...
        savedImDone.setOwner("Chalmers Johnson");
        ...
        try {
            businessService.configure(savedImDone,true, userRepo, pageRepo, designRepo);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        item.setName("Kilo Item " + Giga.getString(3));
        item.setImageUri(Giga.OCEAN_ENDPOINT + Giga.ITEM_IMAGE);
        item.setPrice(new BigDecimal(45));
        item.setQuantity(new BigDecimal(Giga.getNumber(30)));
        item.setWeight(new BigDecimal(Giga.getNumber(48)));
